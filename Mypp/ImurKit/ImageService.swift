...
//            }
            do {
                let imagesData = try NSData(contentsOfFile: getFileUrl().path!, options: .DataReadingUncached)
    ...
    func tempContainerURL(image: UIImage, name: String) -> NSURL? {
        if let containerURL = NSFileManager.defaultManager().containerURLForSecurityApplicationGroupIdentifier("group.com.nustechnology.Demo") {
